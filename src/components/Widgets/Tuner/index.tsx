...
import { Mic, MicOff, Waves, Activity } from 'lucide-react';
      ...
      <div className="flex flex-col w-full max-w-md bg-card border border-border rounded-xl shadow-xl overflow-hidden p-8 space-y-8">
        <h2 className="text-2xl text-center font-bold text-foreground">{t('tuner')}</h2>
          ...
          <Button
            ...
            className={cn('mx-auto mt-6', !isListening && 'bg-accent hover:bg-accent/90')}
          >
          ...
          {isListening && (
            <div className="flex flex-col items-center gap-6 w-full pt-6 border-t border-border mt-6">
              <div className="text-center">
                <div className="text-6xl font-bold text-primary">{note || '—'}</div>
                <div className="text-2xl text-muted-foreground mt-2">{frequency > 0 ? `${frequency} Hz` : '—'}</div>
              </div>
                {/* Deviation Indicator */}
